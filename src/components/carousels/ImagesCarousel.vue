...
<template>
  <div
    class="relative w-full flex flex-col items-center gap-10 py-10"
  >
    <!-- Título -->
    ...
    <!-- Indicadores -->
    <div class="flex items-center gap-2 primary-carousel">
      <span
        v-for="(img, index) in images"
        :key="index"
        class="w-4 h-4 rounded-full cursor-pointer transition-all duration-300"
        :class="currentIndex === index ? 'bg-gray-500 scale-110' : 'bg-gray-300'"
        @click="goTo(index)"
      />
    </div>
